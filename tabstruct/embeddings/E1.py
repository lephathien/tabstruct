...
class GatedRowColumnEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.embedding_dim = config.hidden_size
        self.num_embeddings1 = config.type_vocab_sizes[1]
    ...
    def forward(self, token_type_ids=None):
        embeddings = self.alpha * self.token_type_embeddings_1(token_type_ids[:, :, 1])
        embeddings += self.beta * self.token_type_embeddings_2(token_type_ids[:, :, 2])
        return embeddings
